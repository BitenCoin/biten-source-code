...
#if defined(NDEBUG)
# error "Biten cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0ee1ca2d3fc20faa0972f4312ee05bb6deca001533fb2a9000417f0da5876d3a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Biten: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Biten Signed Message:\n";
...
{
    // Biten: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 300 * COIN;
            ...
            if(nHeight == 1)
            {
            nSubsidy = 2600008 * COIN;
            }

    // Subsidy is cut in half every 212334 blocks
    nSubsidy >>= (nHeight / 212334);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Biten: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Biten: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1515662430, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Twisted radio waves from deep space";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 300 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04dc55b16ea3e864cd512a0e7ab46755c5fc82927c13fd528ed62100aa86d6e3b73b33f2d144748baf2e2f06b6ada7e817028496508ae3178f5740f8158fd0be88") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1515662430;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 5200760;
        ...
        assert(block.hashMerkleRoot == uint256("0x39d63683e5b7b4353ddbdbe11662e9d8419808022bce4837d03ad67d6a1f00f5"));
...
unsigned char pchMessageStart[4] = { 0x80, 0xce, 0xcd, 0x89 }; // Biten: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitenMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitenMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitenMiner(CWallet *pwallet)
{
    printf("BitenMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("biten-miner");
    ...
    try { loop {
        ...
        printf("Running BitenMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitenMiner, pwallet));
}
